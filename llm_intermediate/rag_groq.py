...
def _embed_texts(texts: List[str]) -> List[List[float]]:
    # In deployment, since we are moving away from local Ollama and Groq doesn't natively do
    # embeddings we just fall back gracefully to our fast local hash representation for the RAG lookup
...
def _resolved_collection_name(collection_name: Optional[str]) -> str:
    ...
    if collection_name and collection_name != DEFAULT_COLLECTION_BASE:
        return base
    return f"{DEFAULT_COLLECTION_BASE}__hash-512"
...
"""
    ...
    try:
        ...
        chat_completion = client.chat.completions.create(
            ...
            model="llama3-8b-8192",  # Default fast and cheap Groq model
            temperature=0.15,
            max_tokens=800,
        ...
        print(f"Deployment Groq RAG error: {e}")
