...
from rag import build_index_from_rules, rag_summary, _fallback_summary, gauge_narrative_summary
...
@app.route("/js")
def home():
    return "Credit Risk Evaluation API is running. Use POST / to evaluate entities."
...
def _startup_reindex_background():
    ...
    def job():
        ...
        try:
            rules_rows = df_rules.to_dict(orient="records")
            result = build_index_from_rules(rules_rows)
            print(f"Startup RAG reindex OK: {result}")
        except Exception as exc:
            print(f"Startup RAG reindex failed: {exc}")
